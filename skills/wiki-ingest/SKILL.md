---
name: wiki-ingest
description: Ingest a research paper or document into the LLM Wiki. Full pipeline - parse PDF, write source page, self-check, QA, fix, publish, update concepts/index/log. One paper at a time.
version: 0.2.0
---
...
```
Parse → Draft → Self-Check → QA → Fix → publish → Update Network → Contradiction Check → Log
```
...
| Result | Action |
|--------|--------|
| ≥ 7.0 PASS | → Step 7 (publish) |
| < 7.0 FAIL | → Step 6 (Fix) |

**Only re-QA if first QA timed out without producing a report.** Fixed drafts publish without re-QA.
...
Three mandatory updates after every publish:
...
After every **10th publish** (LLM-0050, LLM-0060, LLM-0070, ...), spawn a concept revision sub-agent.
