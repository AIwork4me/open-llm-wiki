...
def render_status(status: dict[str, Any]) -> str:
    ...
    return (
        ...
        "### Sources by Status\n\n"
        "```dataview\n"
        "TABLE title, status, qa_verdict, claims_total, claims_supported\n"
        "FROM \"sources\" OR \"drafts\"\n"
        "SORT updated_at DESC, updated DESC\n"
        "```\n\n"
        "### Sources by QA Verdict\n\n"
        "```dataview\n"
        "TABLE title, qa_verdict, claims_total\n"
        "FROM \"sources\"\n"
        "WHERE qa_verdict\n"
        "SORT qa_verdict ASC\n"
        "```\n\n"
        "### Concepts with Claim Counts\n\n"
        "```dataview\n"
        "TABLE title, supporting_claims, contradicted_claims, stale_claims\n"
        "FROM \"concepts\"\n"
        "SORT updated_at DESC, updated DESC\n"
        "```\n\n"
        "### Sources by Concept\n\n"
        "```dataview\n"
        "TABLE title, status, claims_total\n"
        "FROM \"sources\"\n"
        "WHERE contains(concepts, this.concept_id) OR contains(tags, this.concept_id)\n"
