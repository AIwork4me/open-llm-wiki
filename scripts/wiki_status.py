...
def top_reading_links(items: list[dict[str, str]], empty: str, limit: int = 5) -> str:
    if not items:
        return f"- {empty}"
    return "\n".join(f"- {link(item['path'], item['title'])}" for item in items[:limit])
...
def render_status(status: dict[str, Any]) -> str:
    ...
    return (
        ...
        "## Start Here\n\n"
        "Use this vault as a reading wiki first, then resolve review work before writeback.\n\n"
        "| Read | Current Entry Points |\n"
        "| --- | --- |\n"
        f"| Sources | {counts['stable_sources']} stable source pages. Start with {top_reading_links(status['recent_sources'], 'no source pages yet.', limit=3).replace(chr(10), '<br>')} |\n"
        f"| Concepts | {counts['concepts']} concept pages. Start with {top_reading_links(status['recent_concepts'], 'no concept pages yet.', limit=3).replace(chr(10), '<br>')} |\n"
        f"| Review required | {counts['claims_needing_review']} claims and {counts['science_review_queue']} science-review items need human review before trusted synthesis. |\n\n"
        f"{action_panel}\n"
        "## Pipeline Status\n\n"
